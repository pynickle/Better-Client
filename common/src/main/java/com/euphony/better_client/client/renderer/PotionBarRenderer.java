...
public class PotionBarRenderer {
    ...
    private static void better_client$drawRow(GuiGraphicsExtractor graphics, MobEffectInstance effect, int x, int y) {
        ...
        graphics.outline(barX, barY, BAR_WIDTH, BAR_HEIGHT, 0x99FFFFFF);
    }

    private static void better_client$drawAmplifierBadge(GuiGraphicsExtractor graphics, int amplifier, int x, int rowY) {
    ...
    private static void better_client$drawRomanAmplifier(GuiGraphicsExtractor graphics, int level, int x, int y) {
        ...
        graphics.text(font, text, 0, 0, AMP_COLORS[AMP_COLORS.length - 1], false);
